...
class VHCR(nn.Module):
    def __init__(self, config):
        ...
        self.unk_sent = nn.Parameter(torch.randn(
            context_input_size - config.z_conv_size))
    ...
    def sent_prior(self, context_outputs, z_conv):
        ...
        h_prior = self.sent_prior_h(
            torch.cat([context_outputs, z_conv], dim=1))
    ...
    def sent_posterior(self, context_outputs, encoder_hidden, z_conv):
        h_posterior = self.sent_posterior_h(
            torch.cat([context_outputs, encoder_hidden, z_conv], 1))
    ...
    def forward(self, sentences, sentence_length,
        ...
        if not decode:
            if self.config.sentence_drop > 0.0:
                indices = np.where(np.random.rand(max_len)
                                   < self.config.sentence_drop)[0]
                if len(indices) > 0:
            ...
            z_conv = conv_mu_posterior + \
                torch.sqrt(conv_var_posterior) * conv_eps
            log_q_zx_conv = normal_logpdf(
                z_conv, conv_mu_posterior, conv_var_posterior).sum()

            log_p_z_conv = normal_logpdf(
                z_conv, conv_mu_prior, conv_var_prior).sum()
            ...
            log_q_zx_sent = normal_logpdf(
                z_sent, sent_mu_posterior, sent_var_posterior).sum()

            log_p_z_sent = normal_logpdf(
                z_sent, sent_mu_prior, sent_var_prior).sum()
